...
```sh
brew install cmake git rapidjson lz4 zlib glfw
```
...
```sh
sudo apt install git build-essential cmake libuuid1 rapidjson-dev liblz4-dev zlib1g-dev libglfw3-dev vulkan-sdk
```
...
#### Arch
```sh
sudo pacman -S git gcc make cmake util-linux-libs rapidjson lz4 zlib glfw-wayland vulkan-headers vulkan-validation-layers shaderc
yay -S robin-hood-hashing
```

#### Void
```sh
sudo xbps-install -S git gcc make cmake libuuid-devel rapidjson robin-hood-hashing liblz4-devel zlib-devel glfw-devel Vulkan-Headers Vulkan-ValidationLayers shaderc
```
