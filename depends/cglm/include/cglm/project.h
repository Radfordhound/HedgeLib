...
/*!
 * @brief maps the specified viewport coordinates into specified space [1]
 *        using zero-to-one clip space. The matrix should contain
 *        projection matrix.
 *
 * if you don't have ( and don't want to have ) an inverse matrix then use
 * glm_unproject_zo version. You may use existing inverse of matrix in somewhere
 * else, this is why glm_unprojecti_zo exists to save save inversion cost
 *
 * [1] space:
 *  1- if m = invProj:     View Space
 *  2- if m = invViewProj: World Space
 ...
 *        projection matrix.
 *
 * this is same as glm_unprojecti_zo except this function get inverse matrix
 * for you.
 *
 * [1] space:
 *  1- if m = proj:     View Space
...
CGLM_INLINE
void
glm_unproject_zo(vec3 pos, mat4 m, vec4 vp, vec3 dest) {
  mat4 inv;
  glm_mat4_inv(m, inv);
  glm_unprojecti_zo(pos, inv, vp, dest);
}
...
/*!
 * @brief map object coordinates to window coordinates using zero-to-one
 *        clip space.
 *
 * Computing MVP:
 *   glm_mat4_mul(proj, view, viewProj);
...
#  define glm_unprojecti_zo glmc_unprojecti_zo
#  define glm_unproject     glmc_unproject
#  define glm_unproject_zo  glmc_unproject_zo
#  define glm_project       glmc_project
#  define glm_project_zo    glmc_project_zo
#endif /* C89 interface */
